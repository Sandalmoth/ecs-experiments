...
fn bench5(alloc: std.mem.Allocator) !void {
    ...
    for (6..22) |log_n| {
        ...
        var s0 = try Storage(u32, u32, 2048, 1024, 0.7).init(alloc);
        defer s0.deinit();
        var s1 = try Storage(u32, u32, 2048, 1024, 0.7).init(alloc);
        defer s1.deinit();
        var s2 = try Storage(u32, u32, 2048, 1024, 0.7).init(alloc);
        defer s2.deinit();
